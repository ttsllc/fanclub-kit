 ...
 */
export function memberCanAccess(config, member, requiredTier) {
 ...
 */
export function effectiveTier(member) {
    if (member.tierId === 'free')
        return 'free';
    if (!member.tierExpiresAt)
        return member.tierId;
    if (new Date(member.tierExpiresAt).getTime() < Date.now())
        return 'free';
    return member.tierId;
}
