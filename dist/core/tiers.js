 ...
 */
export function defineFanclub(input) {
    if (!input.tenantId)
        throw new Error('fanclub-kit: tenantId is required');
    if (input.tiers.length === 0)
        throw new Error('fanclub-kit: at least one tier is required');
    const seen = new Set();
    for (const t of input.tiers) {
        if (seen.has(t.id))
            throw new Error(`fanclub-kit: duplicate tier id "${t.id}"`);
        seen.add(t.id);
        if (t.priceJpy < 0)
            throw new Error(`fanclub-kit: tier "${t.id}" has negative price`);
        if (t.priceJpy > 0 && !t.stripePriceId) {
            throw new Error(`fanclub-kit: tier "${t.id}" is paid (¥${t.priceJpy}) but has no stripePriceId. ` +
    ...
    if (input.inheritance) {
        for (const [from, includes] of Object.entries(input.inheritance)) {
            ...
            for (const target of includes ?? []) {
                if (!seen.has(target)) {
                    throw new Error(`fanclub-kit: inheritance target "${target}" (from "${from}") is not a defined tier`);
                }
            }
        }
    }
