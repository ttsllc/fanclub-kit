...
export async function signupAction(
  ...
  if (!agreeTos || !agreePrivacy) return { error: 'consent_required' };
  ...
  if (!userId) return { error: 'auth_no_user_id' };
 ...
 */
export async function getCurrentMember(config: FanclubConfig) {
  const supabase = await createSupabaseServerClient();
  const {
    data: { user },
  ...
  return {
    ...
    tierExpiresAt: (data.tier_expires_at as string | null) ?? null,
